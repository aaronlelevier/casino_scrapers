...
class FactoryTests(TestCase):
    ...
    def test_create_category_type(self):
        category_type = factory.create_category_type()

        self.assertIsInstance(category_type, CategoryType)

    def test_create_category_trade(self):
        category_type = factory.create_category_type()

        category_trade = factory.create_category_trade(type=category_type)

        self.assertIsInstance(category_trade, CategoryTrade)
        self.assertEqual(category_trade.type_name, category_type.name)

    def test_create_category_issue(self):
        category_trade = factory.create_category_trade()

        category_issue = factory.create_category_issue(trade=category_trade)
