...
def create_phone_numbers(domino_location, related_instance):
    ...
    new_phone_length = PhoneNumber._meta.get_field('number').max_length

    if domino_location.telephone and len(domino_location.telephone) <= new_phone_length:
    ...
    if domino_location.carphone and len(domino_location.carphone) <= new_phone_length:
        ph_type = ph_types.get(name='cell')
        PhoneNumber.objects.create(content_object=related_instance, object_id=related_instance.id,
            number=domino_location.carphone, type=ph_type)

    if domino_location.fax and len(domino_location.fax) <= new_phone_length:
