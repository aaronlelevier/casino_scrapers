...
var TabNewRoute = Ember.Route.extend({
    ...
    afterModel(model, transition) {
        const model_name = this.get('modelName');
        if(model.model.get('new_id')) {
            this.get('tabList').createTab(model.model.get('id'), this.routeName, model_name, this.get('templateModelField'), this.get('redirectRoute'), this.get('store').find(model_name, {new:true}).get('length'));
        }
    },
    actions: {
        parentAction(tab){
