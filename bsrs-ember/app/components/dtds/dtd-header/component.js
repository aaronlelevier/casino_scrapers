...
export default Ember.Component.extend({
  ...
  _dtd_model: Ember.computed(function(){
    const filter = (dtdHeader) => {
      return true;
    };
    return this.get('store').find('dtd-header', filter);
  }),
  message: Ember.computed('_dtd_model.[]', function(){
    const model = this.get('_dtd_model');
    if(model.get('length') > 0 && !model.objectAt(0).get('detail_model')){
      return this.get('store').findOne('dtd-header').get('message'); 
    }
  }),
  actions: {
    togglePreview(){
