...
var LocationLevelChildrenSelect = Ember.Component.extend({
    ...
    actions: {
        change(location_level) {
            const location_level_pk = location_level.get('id');
            const current_location_level = this.get('model');
            const children = current_location_level.get('children_fks') || [];
            if(Ember.$.inArray(location_level_pk, children) > -1) {
                const children_removed = children.filter((child) => {
