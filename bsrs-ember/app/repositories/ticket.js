...
var TicketRepo = Ember.Object.extend(GridRepositoryMixin, FindByIdMixin, CRUDMixin, {
  ...
  patch(destination_id, model, link) {
    //model is the ticket
    return PromiseMixin.xhr(`${PREFIX}/dt/${destination_id}${TICKET_URL}/`, 'PATCH', {data: JSON.stringify(model.patchSerialize(link))}).then((response) => {
      model.save();
      model.saveRelated();
      return response;
    });
