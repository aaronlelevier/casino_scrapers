...
test('ticket-category m2m added including parent id for categories without a fat parent model', (assert) => {
    ...
    assert.deepEqual(categories.objectAt(1).get('children_fks'), [CATEGORY_DEFAULTS.idPlumbing, CATEGORY_DEFAULTS.idTwo]);
    assert.equal(categories.objectAt(0).get('id'), CATEGORY_DEFAULTS.idPlumbing);
    assert.equal(categories.objectAt(0).get('parent_id'), CATEGORY_DEFAULTS.idOne);
    assert.deepEqual(categories.objectAt(0).get('children_fks'), [CATEGORY_DEFAULTS.idPlumbingChild]);
...
test('ticket-category m2m is set up correctly using deserialize single (starting with no m2m relationship)', (assert) => {
    ...
    assert.equal(categories.objectAt(0).get('id'), CATEGORY_DEFAULTS.idPlumbing);
    assert.equal(categories.objectAt(0).get('name'), CATEGORY_DEFAULTS.nameRepairChild);
    assert.equal(categories.objectAt(0).get('parent_id'), CATEGORY_DEFAULTS.idOne);
...
test('attachment added for each attachment on ticket (when ticket has existing attachments)', (assert) => {
    ticket = store.push('ticket', {id: TICKET_DEFAULTS.idOne, ticket_attachments_fks: [TICKET_DEFAULTS.attachmentTwoId], previous_attachments_fks: [TICKET_DEFAULTS.attachmentTwoId]});
    ticket.save();
    store.push('attachment', {id: TICKET_DEFAULTS.attachmentTwoId});
