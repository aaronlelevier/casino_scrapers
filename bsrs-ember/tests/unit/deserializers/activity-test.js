...
test('activity with only cc_add is deserialized correctly', (assert) => {
  ...
  assert.equal(store.find('activity').get('length'), 1);
  assert.equal(store.find('activity/person').get('length'), 1);
  const cc_add = store.find('activity').objectAt(0);
  assert.equal(cc_add.get('person').get('id'), PD.idOne);
  assert.equal(cc_add.get('type'), 'cc_add');
  assert.equal(cc_add.get('to'), undefined);
  assert.equal(cc_add.get('from'), undefined);
  assert.equal(cc_add.get('added').get('length'), 2);
  assert.equal(cc_add.get('added').objectAt(0).get('id'), PD.idBoy);
  assert.equal(cc_add.get('added').objectAt(1).get('id'), PD.idSearch);
  assert.notOk(cc_add.get('content'));
  assert.deepEqual(store.find('activity/cc-add').objectAt(0).get('activities'), [TAD.idCcAddOne]);
  assert.ok(store.find('activity/cc-add').objectAt(0).get('activities')[0] !== store.find('activity/cc-add').objectAt(0).get('activities')[1]);
});

test('multiple deserialization will add to activities array on cc-add', (assert) => {
  let response = TA_FIXTURES.cc_add_only(2, null, 2);
  subject.deserialize(response);
  assert.equal(store.find('activity/cc-add').objectAt(0).get('activities').length, 2);
  assert.ok(store.find('activity/cc-add').objectAt(0).get('activities')[0] !== store.find('activity/cc-add').objectAt(0).get('activities')[1]);
});

test('multiple deserialization will add to activities array on cc-remove', (assert) => {
  let response = TA_FIXTURES.cc_remove_only(2, null, 2);
  subject.deserialize(response);
  assert.equal(store.find('activity/cc-remove').objectAt(0).get('activities').length, 2);
  assert.ok(store.find('activity/cc-remove').objectAt(0).get('activities')[0] !== store.find('activity/cc-remove').objectAt(0).get('activities')[1]);
});

test('multiple deserialization will add to activities array on send-email', (assert) => {
  let response = TA_FIXTURES.send_sms_or_email_only('send_email', 2, null, 2);
  subject.deserialize(response);
  assert.equal(store.find('activity/send-email').objectAt(0).get('activities').length, 2);
  assert.ok(store.find('activity/send-email').objectAt(0).get('activities')[0] !== store.find('activity/send-email').objectAt(0).get('activities')[1]);
});

test('multiple deserialization will add to activities array on send-sms', (assert) => {
  let response = TA_FIXTURES.send_sms_or_email_only('send_sms', 2, null, 2);
  subject.deserialize(response);
  assert.equal(store.find('activity/send-sms').objectAt(0).get('activities').length, 2);
  assert.ok(store.find('activity/send-sms').objectAt(0).get('activities')[0] !== store.find('activity/send-sms').objectAt(0).get('activities')[1]);
});
...
test('activity with send_sms', assert => {
  let response = TA_FIXTURES.send_sms_or_email_only('send_sms', 1);
  subject.deserialize(response);
  assert.equal(store.find('activity').get('length'), 1);
  assert.equal(store.find('activity').objectAt(0).get('ticket'), TD.idOne);
...
test('activity with send_email', assert => {
  let response = TA_FIXTURES.send_sms_or_email_only('send_email', 1);
  subject.deserialize(response);
  assert.equal(store.find('activity').get('length'), 1);
  assert.equal(store.find('activity').objectAt(0).get('ticket'), TD.idOne);
