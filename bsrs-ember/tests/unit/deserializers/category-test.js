...
test('category deserializer returns correct data with existing category (list)', (assert) => {
    ...
    //TODO: add back when figure out what to do with children and their children
    // assert.deepEqual(store.find('category', CATEGORY_DEFAULTS.idTwo).get('children_fks'), []);
    ...
    assert.ok(categories.objectAt(2).get('isNotDirty'), false);
});
