...
import SMSJRD from 'bsrs-ember/vendor/defaults/sendsms-join-recipients';
...
moduleFor('model:sendsms', 'Unit | Model | sendSms', {
  ...
  beforeEach() {
    store = module_registry(this.container, this.registry, ['model:sendsms', 'model:sendsms-join-recipients', 'model:person', 'service:person-current', 'service:translations-fetcher', 'service:i18n']);
    run(() => {
...
test('recipient property should return all associated recipients, and also confirm related and join model attr values', (assert) => {
  run(() => {
    store.push('sendsms-join-recipients', {id: SMSJRD.idOne, sendsms_pk: SMSD.idOne, recipient_pk: PD.idOne});
    sendSms = store.push('sendsms', {id: SMSD.idOne, sendsms_recipient_fks: [SMSJRD.idOne]});
...
test('remove_recipient - will remove join model and mark model as dirty', (assert) => {
  ...
  assert.equal(sendSms.get('sendsms_recipient_ids').length, 1);
  assert.equal(sendSms.get('sendsms_recipient_fks').length, 1);
  ...
  assert.equal(sendSms.get('sendsms_recipient_ids').length, 0);
  assert.equal(sendSms.get('sendsms_recipient_fks').length, 1);
...
test('add_recipient - will create join model and mark model dirty', (assert) => {
  ...
  assert.equal(sendSms.get('sendsms_recipient_ids').length, 2);
  assert.equal(sendSms.get('sendsms_recipient_fks').length, 1);
