...
test('related phone numbers are not dirty when no phone numbers present', (assert) => {
    ...
    var phone_number = store.push('phonenumber', {id: 2, type: 1, person_id: 2});
...
test('person is dirty or related is dirty when model has been updated', (assert) => {
    var person = store.push('person', {id: 1, username: 'scott'});
    var phone_number = store.push('phonenumber', {id: 2, type: 1, person_id: 1});
...
test('when no phone number and new phone number is added and updated, expect isDirty or Related to be true', (assert) => {
    var person = store.push('person', {id: 1});
    store.push('phonenumber', {id: 2, type: 1, person_id: 2});
    assert.ok(person.get('isNotDirty'));
    assert.ok(person.get('isNotDirtyOrRelatedNotDirty'));
    var phone_number = store.push('phonenumber', {id: 3, type: 1, person_id: 1});
    assert.ok(person.get('isNotDirtyOrRelatedNotDirty'));
    phone_number.set('type', 2);
    assert.ok(person.get('isDirtyOrRelatedDirty'));
