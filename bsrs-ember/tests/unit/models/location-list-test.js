...
module('unit: location list test', {
    beforeEach() {
        store = module_registry(this.container, this.registry, ['model:location', 'model:location-list', 'model:location-status-list']);
        run(() => {
            location_detail = store.push('location', {id: LD.idOne, number: 'scoo'});
