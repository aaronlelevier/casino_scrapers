...
moduleForComponent('location-level-children-power-select', 'integration: location-level-children-power-select test', {
    integration: true,
    setup() {
...
test('should render a selectbox with bound options and multiple set to true', function(assert) {
    ...
    this.render(hbs`{{location-level-children-power-select model=model location_level=model options=options}}`);
    let $component = this.$('.t-location-level-location-level-select-power');
    assert.equal($component.find('span.ember-power-select-multiple-option').length, 7);
    //need to click on the options to get all of them
    //assert.equal(this.$('ul > li').length, 8);
});
