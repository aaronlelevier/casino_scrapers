...
moduleForComponent('activity-list', 'amk integration: activity-list', {
    integration: true,
    setup() {
...
test('activity list will dynamically generate a mix of activity types', function(assert) {
    ...
    assert.equal(trim(this.$(`${ACTIVITY_ITEMS}:eq(2)`).text()), `${PD.fullname} uploaded 1 files 15 days ago ${GD.nameTicketAttachment}`);
    ...
    //assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(5)`).text().trim(), `${PD.fullname} changed the priority from ${TD.priorityTwo} to ${TD.priorityOne} 2 months ago`);
