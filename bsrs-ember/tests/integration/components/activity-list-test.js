...
test('activity list will dynamically generate a mix of activity types', function(assert) {
    ...
    assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(2)`).text(), `${PD.fullname} changed the status from ${TD.statusOne} to ${TD.statusTwo}`);
    assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(3)`).text(), `${PD.fullname} changed the priority from ${TD.priorityOne} to ${TD.priorityTwo}`);
    assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(4)`).text(), `${PD.fullname} added person1, person2 to CC`);
    assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(5)`).text(), `${PD.fullname} removed person1, person2 from CC`);
});
