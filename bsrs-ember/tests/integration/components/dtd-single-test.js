...
test('must have one link, cant remove last link, remove btn clears link', function(assert) {
  run(() => {
    ...
    store.push('ticket-priority', {
      id: TP.priorityOneId,
      name: TP.priorityOne,
      links: [LINK.idOne]
    });
    store.push('ticket-status', {
      id: TD.statusOneId,
      name: TD.statusOne,
      links: [LINK.idOne]
    });
...
test('dtd-link category select', function(assert) {
  run(() => {
    ...
    link = store.push('link', {
      id: LINK.idOne,
      model_categories_fks: [TCD.idOne, TCD.idTwo, TCD.idThree]
    });
  });
  this.set('model', dtd);
  this.render(hbs `{{dtds/dtd-single model=model}}`);
