...
test('content for automation generated ticket activity for an SMS', function(assert) {
  ...
  assert.equal(this.$('.t-activity-wrap').text().replace(/[\s\n]+/gm, ''), 
    `${trans.t('automation.actions.sms')}sentto${PD.fullname.replace(/\s+/, '')}${trans.t('general.and')}${PD.fullnameBoy.replace(/\s+/, '')}viafoobar${moment(timestamp).fromNow().replace(/[\s]/g, '')}`);
