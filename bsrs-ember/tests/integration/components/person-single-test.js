...
moduleForComponent('person-single', 'integration: person-single test', {
  ...
  beforeEach() {
    ...
    this.store = module_registry(this.container, this.registry, ['model:person', 'model:role', 'model:currency']);
    translation.initialize(this);
    trans = this.container.lookup('service:i18n');
    const json = translations.generate('en');
    loadTranslations(trans, json);
    run(() => {
      this.store.push('person-current', {id: PD.idOne});
    ...
    run(() => {
      pn_types.forEach((pnt) => {
        this.store.push('phone-number-type', pnt);
      });
    });
    let em_types = [{ 'id': ETD.personalId, 'name': ETD.personalEmail }, { 'id': ETD.workId, 'name': ETD.workEmail }];
    run(() => {
      em_types.forEach((emt) => {
        this.store.push('email-type', emt);
      });
    });
    /* Desktop */
...
test('validation on person username works if clear out username', function(assert) {
  ...
  run(() => {
    this.model = this.store.push('person', {id: PD.idOne, username: 'foo'});
  });
  this.render(hbs`{{people/person-single model=model}}`);
...
test('header populates with username and role name', function(assert) {
  run(() => {
    this.model = this.store.push('person', {id: PD.idOne, username: PD.username, role_fk: RD.idOne});
    this.store.push('role', {id: RD.idOne, name: RD.nameOne, people: [PD.idOne]});
  });
  this.render(hbs`{{people/person-single model=model}}`);
...
test('click on status dropdown and select status', function(assert) {
  run(() => {
    this.model = this.store.push('person', {id: PD.idOne, status_fk: SD.activeId});
    this.store.push('status', {id: SD.activeId, name: SD.activeNameTranslated, people: [PD.idOne]});
    this.store.push('status', {id: SD.inactiveId, name: SD.inactiveNameTranslated});
    this.store.push('status', {id: SD.expiredId, name: SD.expiredNameTranslated});
  });
  this.render(hbs`{{people/person-single model=model}}`);
