...
test('clicking page 2 will load in another set of data as well as clicking page 1 after that reloads the original set of data (both require an additional xhr)', function(assert) {
  ...
  click('.t-page:eq(1) a');
  andThen(() => {
    const categories = this.functionalStore.find('category-list');
    assert.equal(categories.get('length'), 9);
    assert.equal(currentURL(), CATEGORY_LIST_URL + '?page=2');
    assert.equal(find('.t-grid-data').length, PAGE_SIZE-1);
    assert.equal(substring_up_to_num(find('.t-grid-data:eq(1) .t-category-verbose_name').text().trim()), 'cococat');
    pagination2(assert);
  });
  click('.t-page:eq(0) a');
  andThen(() => {
...
test('typing a search will reset page to 1 and require an additional xhr and reset will clear any query params', function(assert) {
  ...
  andThen(() => {
    ...
    assert.equal(find('.t-grid-data:eq(0) .t-category-verbose_name').text().trim(), 'cococat14');
    assert.equal(find('.t-grid-data:eq(1) .t-category-verbose_name').text().trim(), CD.nameOne + '4');
  });
  click('.t-sort-label-dir');
  andThen(() => {
  ...
  andThen(() => {
    ...
    assert.equal(find('.t-grid-data').length, PAGE_SIZE-1);
    assert.equal(substring_up_to_num(find('.t-grid-data:eq(0) .t-category-verbose_name').text().trim()), CD.nameOne);
  });
...
test('loading screen shown before any xhr and hidden after', function(assert) {
  var sort_one = PREFIX + BASE_URL + '/?page=1&ordering=label';
  xhr(sort_one ,"GET",null,{},200,CF.sorted('label'));
...
test('full text searched columns will have a special on css class when active', function(assert) {
  let find_three = PREFIX + BASE_URL + '/?page=1&verbose_name__icontains=7';
  xhr(find_three ,"GET",null,{},200,CF.sorted('name:7'));
  let find_two = PREFIX + BASE_URL + '/?page=1&label__icontains=num&verbose_name__icontains=7';
  ...
  visit(CATEGORY_LIST_URL);
  andThen(() => {
    assert.ok(!find('.t-filter-verbose-name').hasClass('on'));
