...
let detail_xhr, endpoint, model, img_payload;
...
moduleForAcceptance('Acceptance | general dtd file upload test', {
  beforeEach() {
    ...
    model = this.store.find('dtd', DTD.idOne);
...
/* jshint ignore:start */
test('upload will post form data, show progress bar and on save append the attachment to detail and preview', async function(assert) {
...
test('uploading a file, then rolling back should throw out previously associated attachments', async function(assert) {
  const image = {name: 'foo.png', type: 'image/png', size: 234000};
...
test('previously attached files do not show up after file upload', async function(assert) {
  clearxhr(detail_xhr);
  const detail_with_attachment = DTDF.detail(DTD.idOne);
...
test('delete attachment is successful when the user confirms yes (before the file is associated with a dtd)', function(assert) {
...
test('user cannot see progress bar for uploaded attachment that is associated with a dtd (after save)', async function(assert) {
  let image = {name: 'foo.png', type: 'image/png', size: 234000};
  await page.visitDetail();
  ajax(`${PREFIX}/admin/attachments/`, 'POST', new FormData(), {}, 201, img_payload);
  await uploadFile('dtds/dtd-single', 'upload', image, model);
...
// test('file upload supports multiple attachments', async function(assert) {
...
//   await generalPage.save();
//   assert.equal(currentURL(), DETAIL_URL);
//   assert.equal(this.store.find('attachment').get('length'), 2);//need faked out xhr to get 2 attachments in there
//   assert.equal(model.get('isDirty'), false);
//   assert.ok(model.get('isNotDirtyOrRelatedNotDirty'));
// });

test('rolling back should only remove files not yet associated with a given dtd', async function(assert) {
  clearxhr(detail_xhr);
