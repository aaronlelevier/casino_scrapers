...
test('ticket detail shows the activity list including event data (cc_add)', (assert) => {
    ...
    andThen(() => {
        assert.equal(currentURL(), DETAIL_URL);
        assert.equal(find(`${ACTIVITY_ITEMS}`).length, 1);
        assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(0)`).text().trim(), `${PD.fullname} added person1 to CC 15 days ago`);
    });
});
...
test('ticket detail shows the activity list including event data (cc_remove)', (assert) => {
    ...
    andThen(() => {
        assert.equal(currentURL(), DETAIL_URL);
        assert.equal(find(`${ACTIVITY_ITEMS}`).length, 1);
        assert.equal(this.$(`${ACTIVITY_ITEMS}:eq(0)`).text().trim(), `${PD.fullname} removed person1 from CC 15 days ago`);
    });
});
