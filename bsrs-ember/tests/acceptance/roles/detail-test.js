...
// test('sco clicking and typing into selectize for categories will fire off xhr request for all categories', (assert) => {
//     visit(DETAIL_URL);
//     andThen(() => {
//         let role = store.find('role', ROLE_DEFAULTS.idOne);
...
//     let payload = ROLE_FIXTURES.put({id: ROLE_DEFAULTS.idOne, category: category.id});
//     xhr(url, 'PUT', JSON.stringify(payload), {}, 200);
//     click(SAVE_BTN);
//     andThen(() => {
//         assert.equal(currentURL(), ROLE_URL);
