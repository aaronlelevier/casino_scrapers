...
test('visiting admin/location', (assert) => {
    ...
    let response = LOCATION_LEVEL_FIXTURES.detail(LOCATION_DEFAULTS.idOne);
    let payload = LOCATION_LEVEL_FIXTURES.put({id: LOCATION_DEFAULTS.idOne, name: LOCATION_DEFAULTS.nameRegion});
    xhr(DJANGO_DETAIL_URL, 'PUT', JSON.stringify(payload), {}, 200, response);
    fillIn('.t-location-level-name', LOCATION_DEFAULTS.nameRegion);
    andThen(() => {
        let location = store.find('location-level', LOCATION_DEFAULTS.idOne);
        assert.ok(location.get('isDirty'));
    });
    let list = LOCATION_LEVEL_FIXTURES.list();
    list.results[0].name = LOCATION_DEFAULTS.nameRegion;
...
test('when editing name to invalid, it checks for validation', (assert) => {
    ...
    let response = LOCATION_LEVEL_FIXTURES.detail(LOCATION_DEFAULTS.idOne);
    let payload = LOCATION_LEVEL_FIXTURES.put({id: LOCATION_DEFAULTS.idOne, name: LOCATION_DEFAULTS.nameCompany});
    xhr(DJANGO_DETAIL_URL, 'PUT', JSON.stringify(payload), {}, 200, response);
    click(SAVE_BTN);
    andThen(() => {
        assert.equal(currentURL(), LOCATION_LEVEL_URL);
...
test('when user changes an attribute and clicks cancel we prompt them with a modal and then roll back the model', (assert) => {
    ...
    andThen(() => {
        waitFor(() => {
            assert.equal(currentURL(), LOCATION_LEVEL_URL);
            assert.equal(find('.t-modal').is(':hidden'), true);
            let location_level = store.find('location-level', LOCATION_DEFAULTS.idOne);
